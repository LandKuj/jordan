...
public class KDtree<Key extends Comparable<?super Key>, E> {
 ...
 public int incrementDim(int currDim, int keySize) { 
   currDim++; 
   if(currDim == keySize - 1) { 
     currDim = 0; 
   }
   return currDim; 
 }
 ...
 /*
 public void getNeightbors(Key[] key, int r, int dim, BinNode root) { 
   int keyDim = key[dim]; 
   int dimDiff = key[dim] - root.getKey()[dim].getValue(); 
   if(dimDiff < 0) { 
     dimDiff = dimDiff * -1; 
   }
   if(dimDiff <= r) { 
   ...
   if(key[dim] - root.key[dim] < 0) { 
     if(root.getLeft() != null) { 
       getNeightbors(key, r, this.incrementDim(dim, key.length), root.getLeft()); 
     }
   } else { 
     if(root.getRight() != null) { 
       getNeightbors(key, r, this.incrementDim(dim, key.length), root.getRight()); 
     }
   } 
 }
...
//to be implemented 
 public void getNeighbors(Key[] key, int r) {
   this.getNeightbors(key, r, 0, this.root); 
 }
...
*/
 ...
 public static void main(String[] args) {
  ...
  System.out.println(kdt.root.getKey()[0].compareTo(1000)); 
